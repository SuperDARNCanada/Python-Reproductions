	...
	for file in fitfiles:
		...
		if records[0]['stid'] in north_st_ids:
			...
			for i in range(len(records)):
				...
				try:
					gs_count_n[hr] += np.count_nonzero(records[i]['gflg'])
					...
					pass
			# Create the list of # records per hour for this radar X 75, this is possible
			# counts for this radar per hour
			...
			for i in range(len(records)):
				...
				try:
					gs_count_s[hr] += np.count_nonzero(records[i]['gflg'])
					ranges = records[i]['slist']
					pwr = [records[i]['pwr0'][x] for x in ranges]
